...
Процедура РезультатПодбораОблигацийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе 
		...
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ПослеЗакрытияФормыНоменклатуры", ЭтотОбъект, ДополнительныеПараметры);
		ОткрытьФорму("Справочник.Номенклатура.ФормаОбъекта", ПараметрыФормы,,,,, ОписаниеОповещенияОЗакрытии);
	КонецЕсли; 
...
Процедура ПослеЗакрытияФормыНоменклатуры(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Попытка
		Элемент.ТекущиеДанные.Номенклатура = ПолучитьНоменклатуруНаСервере(Элемент.ТекущиеДанные.КодБумаги); 
	Исключение
		// если открыть форму номенклатуры, удалить элемент и закрыть форму обработки, то выдаётся ошибка => пока обойдём через попытка/исключение
	КонецПопытки; 
