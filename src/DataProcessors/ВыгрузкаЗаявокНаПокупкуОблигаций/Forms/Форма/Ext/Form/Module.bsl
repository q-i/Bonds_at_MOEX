	...
	Для Каждого ТекЭлемент Из ПолучитьВсеПодчиненныеЭлементы(Элементы.Данные) Цикл
		Если ТипЗнч(ТекЭлемент) = Тип("ПолеФормы") Тогда
			Если РазрешеноРедактировать.Получить(ТекЭлемент) = Истина Тогда
				ТекЭлемент.ЦветФона = WebЦвета.МятныйКрем;
			Иначе
...
Процедура ЗаполнитьДанныеНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			НоваяСтрока.ПлановаяДоходность = НайденныеСтроки[0].ПлановаяДоходность;
		Иначе
...
Процедура РассчитатьСуммуИКоличествоЗаявкиВСтроке(СтрокаТаблицыДанных) 
	...
	ЦенаЗаявки = ЦенаПокупки / СтрокаТаблицыДанных.НоминальнаяСтоимость * 100; // в %-тах от номинала
	СтрокаТаблицыДанных.ЦенаЗаявки = Цел(ЦенаЗаявки * 100) / 100; // отброс дробной части начиная с 3-го знака после запятой
	СтрокаТаблицыДанных.КоличествоЗаявки = Окр(СуммаЛимитНаОднуБумагу / СтрокаТаблицыДанных.ЦенаЗаявки);
