...
Процедура ЗаполнитьДанныеНаСервере()
	...
	|		Сумма КАК Сумма
	|	) КАК Погашения,
	|	ЕСТЬNULL(Ценообразование.НадбавкаКПлановойДоходности, 0) КАК НадбавкаКПлановойДоходности,
	|	ЕСТЬNULL(Ценообразование.МинЦена, 0) КАК МинЦенаЗаявки,
	|	ЕСТЬNULL(Ценообразование.МаксЦена, 0) КАК МаксЦенаЗаявки
	|ИЗ
	|	Справочник.Номенклатура КАК СпрНоменклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Ценообразование КАК Ценообразование
	|		ПО СпрНоменклатура.Ссылка = Ценообразование.Номенклатура
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			НоваяСтрока.ПлановаяДоходность = НайденныеСтроки[0].ПлановаяДоходность + Выборка.НадбавкаКПлановойДоходности;
...
Процедура РассчитатьСуммуИКоличествоЗаявкиВСтроке(СтрокаТаблицыДанных) 
	...
	Если ИтеративныйПодгонПодПлановуюДоходность Тогда
		...
		КонецЦикла;
		Если НЕ Успех Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось итеративно подогнать под плановую доходность цену для бумаги " + СтрокаТаблицыДанных.Код + "!",, "Данные[" + Формат(Данные.Индекс(СтрокаТаблицыДанных), "ЧГ=") + "].Код");
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СтрокаТаблицыДанных.МинЦенаЗаявки)
		И (СтрокаТаблицыДанных.ЦенаЗаявки < СтрокаТаблицыДанных.МинЦенаЗаявки) Тогда
		СтрокаТаблицыДанных.ЦенаЗаявки = СтрокаТаблицыДанных.МинЦенаЗаявки;
		РассчитатьРасчетнуюДоходностьВСтроке(СтрокаТаблицыДанных);
	КонецЕсли;
	Если ЗначениеЗаполнено(СтрокаТаблицыДанных.МаксЦенаЗаявки)
		И (СтрокаТаблицыДанных.ЦенаЗаявки > СтрокаТаблицыДанных.МаксЦенаЗаявки) Тогда
		СтрокаТаблицыДанных.ЦенаЗаявки = СтрокаТаблицыДанных.МаксЦенаЗаявки;
		РассчитатьРасчетнуюДоходностьВСтроке(СтрокаТаблицыДанных);
	КонецЕсли;
	
	РассчитатьКоличествоЗаявки(СтрокаТаблицыДанных, СуммаЛимитНаОднуБумагу);
	
КонецПроцедуры

&НаКлиенте
Процедура РассчитатьКоличествоЗаявки(СтрокаТаблицыДанных, СуммаЗаявки)
	
	СтрокаТаблицыДанных.КоличествоЗаявки = Окр(СуммаЗаявки / (СтрокаТаблицыДанных.ЦенаЗаявки * СтрокаТаблицыДанных.НоминальнаяСтоимость / 100));
...
Процедура УстановитьВидимость()
	...
	Элементы.ГруппаПараметрыТейкПрофит.Видимость = (ВидЗаявки = "Тэйк-профит");
	Элементы.ГруппаПараметрыСтопЦенаПоДругойБумаге.Видимость = (ВидЗаявки = "Стоп-цена по другой бумаге");
...
Процедура ВыгрузитьЗаявки(Команда)
	...
	Для Каждого СтрокаТаблицы Из Данные Цикл
		...
		ДатаДействияПлан = СрокДействияЗаявки;
		Если НЕ ЗначениеЗаполнено(ДатаДействияПлан) И ДатаЗаявок > ТекущаяДата() Тогда
			// выставляем заявки в будущую дату => нужно в качестве даты окончания срока действия указать дату заявок
			ДатаДействияПлан = ДатаЗаявок;
		КонецЕсли; 
		ДатаОкончанияСрока = Мин(ДатаДействияПлан, СтрокаТаблицы.ДатаПогашения - (ТоргиВРежимеТПлюс * Сутки));
		...
		ИначеЕсли ВидЗаявки = "Стоп-цена по другой бумаге" Тогда
			...
			Стр = Стр
				...
				+ "STOPPRICE_CONDITION="	+ "<=" + ";"
				+ "STOPPRICE="				+ Формат(ЦенаЗаявки + ДельтаСтопЦены, ФорматЦены) + ";"
				+ "PRICE="					+ Формат(ЦенаЗаявки, ФорматЦены) + ";"
			;
			Если ЗначениеЗаполнено(ДатаОкончанияСрока) Тогда
				Стр = Стр
...
&НаКлиенте
Процедура ИзменитьСуммуЛимитУВыделенныхСтрок(Команда)
	
	ДополнительныеПараметры = Новый Структура;
	ОписаниеОповещения = Новый ОписаниеОповещения("ВводНовойСуммыЛимитДляВыделенныхСтрокЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	ПоказатьВводЧисла(ОписаниеОповещения, СуммаЛимитНаОднуБумагу);
	
КонецПроцедуры
 
&НаКлиенте
Процедура ВводНовойСуммыЛимитДляВыделенныхСтрокЗавершение(ВведенноеЧисло, ДополнительныеПараметры) Экспорт
	
	Если ВведенноеЧисло = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ВыделеннаяСтрока Из Элементы.Данные.ВыделенныеСтроки Цикл
		СтрокаТаблицыДанных = Данные.НайтиПоИдентификатору(ВыделеннаяСтрока);
		РассчитатьКоличествоЗаявки(СтрокаТаблицыДанных, ВведенноеЧисло);
	КонецЦикла;
